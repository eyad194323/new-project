...
module.exports = {
    ...
    .setName('set-products')
    .setDescription('تعيين روم وصول طلبات المنتجات او رول منشن الطلب')
...
async execute(interaction) {
    if (!owner.includes(interaction.user.id)) return;
        ...
        .setTimestamp(Date.now())
        .setColor('#000000')
                .setTitle(`**تم تحديد <#${id}> روم لطلبات المنتجات**`)
                if(db.has(`products_${thing}_id`)) db.delete(`products_${thing}_id`)
                db.set(`products_${thing}_id` , id)
        ...
        .setTimestamp(Date.now())
        .setColor('#000000')
                .setDescription(`**تم تحديد <@&${id}> رول منشن طلبات المنتجات**`)
                if(db.has(`products_${thing}_id`)) db.delete(`products_${thing}_id`)
                db.set(`products_${thing}_id` , id)
